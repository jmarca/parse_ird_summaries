    ...
    function save_chunk(data,next){
                            ...
                            ,function(err){
                                 if(err) {
                                     ...
                                     if(! /duplicate key value/.test(err)){
                                         ...
                                         throw new Error(err)
                                     //}else{
                                         //console.log('duplicate key: '+ stmt)
                                     }
                                 }
                                 return cb()
                             })
                        ...
                        ,function(err){
                             ...
                             if(err){
                                 if(/duplicate key value/.test(err)){
                                     ...
                                     console.log('all at once failed for '+insert_string + rows.join(','))
                                     console.log(err)
                                     throw new Error(err)
                                     // bleh
                                 }
        ...
        pg.connect(connectionString, function(err, client, done) {
            ...
            .await(function(err,result){
                if(err || ! result){
                    console.log('all at once failed for '+insert_string+' for '+values.length+'values (from '+values[0]+' to '+values[values.length - 1]+'; try smaller chunks')
                    //return next(err)
                    ...
                    console.log('no problems saving for '+insert_string+' for '+values.length+'values')
                    done()
                    next()
                }
                return null
