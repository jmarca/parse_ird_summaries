...
function setup_file_parser (options ){
    ...
    function parse_file(file,cb){
        ...
        var table_results=[] // gather up table results
            ...
            while (null !== (line = stream.read())) {
                ...
                if(result === null){
                    ...
                    if( states[state].parsed_something() // if true, seen valid lines
                        ...
                        // if there were no strange values in the
                        // table, save the data
                        if(states[state].good_read())
                            the_collectors[state](table_results)

                        // reset the aggregator for the next table
                        table_results = []
                    ...
                    if(_.isArray(result) && result.length>0){
                        ...
                        if(state == 1){
                            table_reults = result.reduce(speed_vs_class_row_reducer,table_results)
                        }else{
                            table_results = result.reduce(hourly_row_reducer,table_results)
                        }
